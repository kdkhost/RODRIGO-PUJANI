<section id="{{ $embedded ? 'areas' : 'areas-de-atuacao' }}" class="py-24 lg:py-36 relative" style="background:#0B0C10;">
    ...
    <div class="max-w-7xl mx-auto px-6 lg:px-16 relative">
        <div class="text-center mb-16">
            ...
            <h2 class="font-display leading-tight aos delay-100" style="font-size:clamp(2.2rem,5vw,4rem);font-weight:300;">Áreas de <span class="text-gold-gradient font-semibold">Atuação</span></h2>
            <p class="text-cream/40 max-w-lg mx-auto mt-4 aos delay-200">Cobertura jurídica abrangente com especialistas dedicados em cada área do direito brasileiro.</p>
        </div>

        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            @foreach($featuredAreas as $area)
                @php($badges = collect(preg_split('/[,|]/', (string) ($area->highlight ?: '')))->map(fn ($item) => trim($item))->filter()->take(3))
                <article class="card-glass p-8 aos delay-{{ (($loop->index % 3) + 1) * 100 }}">
