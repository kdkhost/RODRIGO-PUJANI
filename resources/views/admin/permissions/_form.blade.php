...
<form action="{{ $isEdit ? route($routeBase.'.update', $record->id) : route($routeBase.'.store') }}" method="POST" data-ajax-form class="admin-premium-form">
    ...
    <div class="row g-3">
        <div class="col-md-7">
            <label class="form-label">Nome da permissao</label>
            <input type="text" name="name" class="form-control" value="{{ old('name', $record->name) }}" placeholder="Ex.: pages.manage" required>
            <div class="invalid-feedback" data-error-for="name"></div>
            <div class="form-text">Use nomes modulares, como <code>modulo.acao</code>, para manter o painel organizado.</div>
        </div>
    ...
    <div class="d-flex justify-content-end gap-2 mt-4">
        ...
        <button type="submit" class="btn btn-primary">
            <i class="bi bi-check2-circle me-1"></i>Salvar permissao
        </button>
    </div>
</form>
