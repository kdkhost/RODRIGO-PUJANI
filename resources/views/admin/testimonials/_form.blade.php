...
<form action="{{ $isEdit ? route($routeBase.'.update', $record->id) : route($routeBase.'.store') }}" method="POST" data-ajax-form enctype="multipart/form-data">
    ...
    <div class="row g-3">
        ...
        <div class="col-md-3">
            ...
            <input type="file" name="image" class="form-control" data-filepond data-accepted="image/png,image/jpeg,image/webp,image/svg+xml" data-current-url="{{ $record->image_path ? site_asset_url($record->image_path) : '' }}" data-current-name="{{ $record->image_path ? basename($record->image_path) : '' }}">
            @if($record->image_path)
                <div class="admin-brand-preview-card mt-3">
                    ...
                    <div class="admin-brand-preview-image mt-2" style="min-height: 180px;">
                        <img src="{{ site_asset_url($record->image_path) }}" alt="{{ $record->author_name }}" style="max-height: 160px; border-radius: 0.8rem;">
                    </div>
                    <div class="mt-2 small text-muted">Imagem atual: <a href="{{ site_asset_url($record->image_path) }}" target="_blank" rel="noopener">{{ $record->image_path }}</a></div>
                </div>
            @endif
        </div>
