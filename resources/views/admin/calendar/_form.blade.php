...
<form action="{{ $isEdit ? route('admin.calendar.update', $record) : route('admin.calendar.store') }}" method="POST" data-ajax-form>
    ...
    <div class="row g-3 admin-premium-form">
        ...
        <div class="col-md-4">
            ...
            <select name="status" class="form-select" required>
                @foreach ($statuses as $status)
                    <option value="{{ $status }}" @selected(old('status', $record->status ?: 'scheduled') === $status)>{{ ucfirst($status) }}</option>
                @endforeach
            </select>
        </div>
        <div class="col-md-4">
            ...
            <select name="visibility" class="form-select" required>
                @foreach ($visibilities as $visibility)
                    <option value="{{ $visibility }}" @selected(old('visibility', $record->visibility ?: 'team') === $visibility)>{{ ucfirst($visibility) }}</option>
                @endforeach
            </select>
        </div>
        <div class="col-md-4">
        ...
        <div class="col-md-4">
            <label class="form-label">Renderizacao</label>
            <select name="display" class="form-select">
                @foreach ($displays as $display)
                    <option value="{{ $display }}" @selected(old('display', $record->display ?: 'auto') === $display)>{{ $display }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="col-md-4 form-check ps-5">
            ...
            <label class="form-check-label" for="calendar_overlap">Permitir sobreposicao</label>
        </div>
